...
const Sidebar: React.FC = () => {
  const { currentView, setView, user, setSearchQuery, setActiveFilter } = useStore();
  ...
  return (
    <aside className="w-64 hidden lg:flex flex-shrink-0 border-r border-gray-100 flex-col justify-between p-4 h-full bg-white sticky top-0">
      <div className="flex flex-col gap-8">
        <div className="px-2 pt-2 cursor-pointer" onClick={() => handleNavClick(ViewMode.Home)}>
        ...
        <div className="flex flex-col gap-3 px-2">
          <p className="text-xs font-semibold uppercase text-text-secondary tracking-wider">Trending</p>
      ...
      <div className="border-t border-gray-100 pt-4 px-2">
        <div onClick={() => handleNavClick(ViewMode.Account)} className="flex items-center gap-3 cursor-pointer hover:opacity-80">
          ...
          <div className="flex flex-col overflow-hidden">
            <span className="text-sm font-medium text-text-main truncate">{user?.name || 'User'}</span>
