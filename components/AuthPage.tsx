      ...
      <div className="w-full max-w-[440px] bg-white rounded-[32px] shadow-2xl shadow-primary/5 p-10 flex flex-col gap-8 border border-gray-100">
        ...
        {view === 'login' && (
          <div className="flex bg-background-subtle p-1 rounded-2xl">
            <button onClick={() => setLoginMode('password')} className={`flex-1 py-2 rounded-xl text-xs font-black transition-all ${loginMode === 'password' ? 'bg-white text-primary shadow-sm' : 'text-text-secondary'}`}>Password</button>
            <button onClick={() => setLoginMode('code')} className={`flex-1 py-2 rounded-xl text-xs font-black transition-all ${loginMode === 'code' ? 'bg-white text-primary shadow-sm' : 'text-text-secondary'}`}>Code</button>
          </div>
        )}

        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            <label className="text-[10px] font-black uppercase tracking-widest text-text-secondary px-1">Email</label>
            <input 
          ...
          {(view === 'register' || (view === 'login' && loginMode === 'code') || view === 'forgot') && (
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between px-1">
                <label className="text-[10px] font-black uppercase tracking-widest text-text-secondary">Code</label>
                <button type="button" onClick={handleSendCode} className="text-[10px] font-black uppercase text-primary hover:underline">{isSendingCode ? 'Sending...' : 'Get Code'}</button>
              </div>
          ...
          <button type="submit" className="w-full py-4 bg-primary text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-primary/20 hover:scale-[1.02] transition-transform active:scale-95 mt-2">
            {view === 'login' ? 'Sign In' : view === 'register' ? 'Register' : 'Reset'}
          </button>
        </form>
